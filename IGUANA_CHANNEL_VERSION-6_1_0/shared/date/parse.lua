...
-- Convert a time value to a simple day format with no time
function string:DAY(fmt)
   local t = dateparse.parse(self,fmt)
   return t and os.date('%Y-%m-%d', t)
end

-- Converts a time to the specified date format
function string:FORMAT_DATETIME(fmtIn, fmtOut)
   local t = dateparse.parse(self, fmtIn)
   return t and os.date(fmtOut,t)
end

function string:ISO8601(timezone)
   local t = dateparse.parse(self)
   timezone = timezone or 'Z'  -- Default to UTC if no timezone is provided
   return t and os.date('%Y-%m-%dT%H:%M:%S', t) .. timezone
end
