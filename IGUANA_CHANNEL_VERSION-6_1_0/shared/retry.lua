...
function retry.call(P)--F, A, RetryCount, Delay)
   ...
   for i =1, RetryCount do
      local R = {pcall(Func, unpack(Args))}
      Success = R[1]
      code = R[2]  --ErrMsgOrReturnCode = R[2]
      if ErrorFunc then
         ErrorFunc(unpack(R))
      elseif Success then
         -- Function call did not throw an error 
   ...
   error(RETRIES_FAILED_MESSAGE..' HTPP error: '.. tostring(code) ..'. Stopping channel.\n')--..tostring(ErrMsgOrReturnCode)) 
end
   
  -- -- stop channel if retries are unsuccessful
--   iguana.setChannelStatus{text=RETRIES_FAILED_MESSAGE}
   --error(RETRIES_FAILED_MESSAGE..' HTTP error code: ' .. tonumber(R[3]) .. '(). Stopping channel.')
--   error(RETRIES_FAILED_MESSAGE..' HTTP error code: ' .. tonumber(ErrMesgOrReturnCode).. '(). Stopping channel.\n') --..tostring(ErrMesgOrReturnCode))
--  end
